...
use crate::args::{
    ArmStopPointsArgs, AttachArgs, CheckStaleArgs, ClearBreakpointArgs, EvaluateArgs, EvaluateChainArgs,
    ForceReturnArgs, GetLastEventArgs, GetStackArgs, GetTracesArgs, LaunchArgs, ListClassesArgs,
    ListFieldsArgs, ListInstancesArgs, ListMethodsArgs, ListStopPointsArgs, ListThreadsArgs, PopFrameArgs,
    ReloadClassArgs, ResumeThreadArgs, RunNamedQueryArgs, SetBreakpointArgs, SetExceptionBreakpointArgs,
    SetMethodBreakpointArgs, SetMonitorStopArgs, SetValueArgs, SetWatchpointArgs, SourceArgs, StepArgs,
    SuspendThreadArgs, ThreadDumpArgs, ToggleBreakpointArgs, UpdateStopPointArgs,
};
...
/// What `session_id` does, written once for all forty tools.
///
...
fn stop_point_tools() -> Vec<Tool> {
    vec![
        ...
        Tool {
            name: "debug.export_investigation".to_string(),
            description: "Emit the WHOLE investigation as one Markdown report — the attach target and VM version, every stop point with its measured capture cost, every trace snapshot still in the buffer, the drift verdicts and the budget disarms. Call it when the finding is the deliverable: an investigation is otherwise readable exactly once, by exactly one reader, and the evidence a shared-JVM diagnosis rests on is usually \"here are 40 snapshots showing the tenant arriving null on 11 of them\", which is a thing to attach to a ticket rather than paraphrase — and paraphrase is all that survives a context window being summarised. IT IS NOT REDACTED, AND THE REPORT SAYS SO IN ITS OWN FIRST PARAGRAPH. Snapshots hold whatever the debuggee's variables held: request and response payloads, bearer tokens, credentials in a byte[], customer records. Nothing is altered, because a pattern-based redactor that misses one secret is WORSE than none — its output implies the file was cleaned. Reviewing it before attaching it anywhere is the caller's job, and the warning names payloads, tokens and byte[] credentials specifically rather than saying \"may contain sensitive data\", because a generic warning is one nobody acts on. Until now this content went to one caller who had already seen it; a file on a ticket is a different exposure, and that is the change to weigh. IT IS THE SESSION AND NOT THE BUFFER, which is why it is a tool of its own rather than a flag on debug.get_traces: the stop points, their costs, the attach target and the VM version are what make the snapshots interpretable, and none of that is in the buffer. The stop-point section is the same renderer debug.list_stop_points uses, so the report cannot drift from the listing. IT NEVER CLEARS ANYTHING — the buffer, the stop points and the disarm notices are exactly as they were, and debug.get_traces with clear:true is still the only thing that empties them. Nor does it let a long trace outlive the ring: it CANNOT, because that would mean this server writing files as the buffer fills, which BP-8 declined. What it does instead is say how many snapshots are already gone, since a ring capped at 500 drops the earliest hits of a long trace and a folded rethrow replaces an earlier record — neither recoverable, and a partial record that does not admit it is a misleading one. Markdown rather than JSON or HTML because the consumer is a model or a ticket, and a JSON dump would lose the prose that makes a snapshot mean something.".to_string(),
            input_schema: empty(),
        },
        Tool {
            name: "debug.arm_stop_points".to_string(),
            description: "Re-arm a whole investigation from a stop-point set that debug.list_stop_points produced with export:true — the inverse of that export, and the reason a repeat investigation does not have to be re-armed by hand. Pass the block back verbatim as `set`; the ```json fence it was shown inside is accepted, because that is how the export renders it. EVERY ENTRY GOES THROUGH THE ORDINARY debug.set_* HANDLER FOR ITS KIND, which is the design rather than an implementation detail: a set IS the list of those calls, so replaying it inherits every refusal, clamp, capability check, deferral and read-only rule those tools already enforce, instead of a second arming path that could drift from them — and the rules it would drift on are the ones that keep a shared JVM alive. So a broad suspending method exit is refused here exactly as it is there, and an invoking condition or trace_expr is refused under read_only exactly as it is there; arming itself is not a write to the debuggee, so read_only does not otherwise apply. IT IS NOT A GENERIC BATCH EXECUTOR. A set may name only the five arming tools, and an entry naming anything else is refused with the whitelist quoted — without that list this would be a way to invoke anything in this server from a blob of JSON. NOTHING ABORTS ON A BAD ENTRY and EVERY OUTCOME IS REPORTED PER ENTRY: the reply leads with `2 armed, 1 deferred, 1 refused` and then names each entry that is not plainly armed, with the tool's own reason. One refused location is a normal batch result, not a reason to leave the other twenty-nine unarmed — and an aggregate that read `4 armed` while one of the four was refused is the silence-reads-as-success failure this reply shape exists to prevent. Deferral is read off the session rather than out of the reply text, so it stays right when the wording changes. AN ENTRY MARKED DISABLED IS NOT ARMED, and this is deliberate rather than a limitation: it was off when you exported it, and arming-then-disabling would arm it for real in between, which on a suspending stop point is long enough to fire. Its arguments are still in the set, so arm it by hand from what is written there. LINES ARE NOT CHECKED AGAINST THE LOADED BYTECODE and the reply says so every time anything armed. A set carries line numbers, which are a claim about the build it came from; against a redeployed one they resolve to whatever is now on that line and arming reports success either way. Run debug.check_stale to settle it — it is a separate call because thirty entries would be thirty round trips, and a set re-armed ten seconds later against the same JVM needs none of them. A set that arms SUSPENDING stop points is warned about and not refused, because this is the one call that can arm many at once, where every other path arms one and you read a reply between each. A set from a NEWER build of this server is refused rather than read optimistically.".to_string(),
            input_schema: to_val(schemars::schema_for!(ArmStopPointsArgs)),
        },
        Tool {
            name: "debug.list_stop_points".to_string(),
            description: "List all active breakpoints, including deferred ones, exception breakpoints, field watchpoints, and wildcard families (bpset_…). A family's line is the only place you can learn what a wildcard has BECOME since you armed it: how many breakpoints it holds now, which classes it armed after the reply you read, and whether it has stopped taking new ones because it is full at max_classes — a full family also PARKS its class-load watch, so it costs the JVM nothing while it is full and starts watching again by itself the moment you clear a member. The line distinguishes all four watch states, because they answer \"will this catch the class my next deployment generates?\" differently: watching (yes), parked because full (not until a slot frees), disabled (not until you re-arm the family), and could-not-be-registered (never). Each traced (non-suspending) stop point also reports what it has ACTUALLY cost: the mean capture per hit, the rate hits are arriving at, and the share of the window spent capturing — measured on your JVM rather than taken from the ~720 hits/s figure in the other tools' descriptions, which is 1/mean and so recoverable from the mean reported here. Call it after arming a trace on a hot site to find out whether it is hurting the instance. A traced stop point that has captured nothing says so explicitly rather than reporting zero. A TRACED STOP POINT THAT IS IN FACT FREEZING THE VM IS MARKED `(trace — SUSPEND POLICY OVERRIDDEN)` AND NOT A BARE `(trace)`, with a line naming what escalated it: a JDWP composite carries one suspend policy for the whole event set, the strongest any member asked for, so a suspending stop point at the same bytecode location revokes the non-suspending promise of every trace there. This is the place to look when the answer to \"why did the VM freeze?\" is a stop point you thought was cheap. EVERY stop point reports Hits: N — how many times the JVM has reported it firing — AND ZERO IS PRINTED RATHER THAN OMITTED, because \"armed, no Hits line\" would read as \"this code never ran\" and that is the reading this number exists to remove. It counts HITS, not the hits you were told about: a hit whose condition was FALSE still counts (so `Hits: 400` beside a condition that never matched is a different diagnosis from `Hits: 0`, and they used to look identical), a rethrow of an already-captured exception counts although it does not spend trace_max_hits (so on a traced stop point Hits and the capture count answer different questions rather than repeating one), and a method-exit request counts only exits of the method you ASKED for, not the every-method-of-the-class traffic JDWP delivers it. A METHOD-EXIT STOP POINT WITH A method FILTER THEREFORE REPORTS A SECOND NUMBER BESIDE IT — `Hits: 0 · exits discarded: 3214` — AND THE PAIR IS THE DIAGNOSIS RATHER THAN EITHER HALF. Alone, `Hits: 0` cannot tell \"the code never ran\" from \"it fired constantly and every event was discarded\", because JDWP has no method-name modifier: a ClassMatch delivers every method of a matching class and the ones you did not ask for are dropped HERE, after the debuggee has already paid for them. Both readings printed the identical line until this existed, and the cost of that was measured — a request that went from 3.2 s unarmed to a 240 s read timeout armed still read `Hits: 0`, which sent two full end-to-end runs looking for a hang in the wrong process. So `exits discarded: 0` beside `Hits: 0` now says the class produced no exits at all, a non-zero count beside it says the class is executing and your method is what did not return, and a line under the pair says which of the two you are reading. THE DISCARDED COUNT IS ALSO THE ONLY REPORT OF WHAT ARMING THIS COST: every discarded exit is a notification the JVM raised and a packet it sent over the one connection, so it is the number to read before leaving a method-exit stop point armed on a shared instance. Narrowing class_pattern is the only thing that reduces it. Zero is printed, for the same reason `Hits: 0` is; nothing is printed at all when the request has no method filter, because such a request wants every method and discards none. Counted once per hit and not once per armed location, so a finally line armed at 2 locations reports the number of times it ran. Cumulative across a toggle_stop_point disable and re-arm, since that keeps the same stop point rather than making a new one. A STOP POINT ARMED WITH hit_count IS REPORTED **SPENT** ONCE IT HAS FIRED, which is a third state and not a synonym for disabled: disabled is something YOU did with toggle_stop_point and can undo, spent is something the DEBUGGEE did — JDWP's Count modifier makes the JVM report the Nth occurrence and delete the request itself. Before this existed such a stop point was listed as armed forever, which is the same silence-reads-as-an-answer failure the Hits line above removes. toggle_stop_point re-arms a spent stop point with the same count. PASS export:true TO GET THE SET BACK AS CONTENT YOU CAN KEEP, instead of a listing to read. A stop-point set dies with the process — stdio means the client's lifetime IS the session's — so eight stop points across four classes, with their conditions, trace_expr lists and hit_counts, are gone at disconnect and re-armed by hand tomorrow. The export is the list of debug.set_* calls that would recreate them, which is also exactly what debug.arm_stop_points takes back. THIS SERVER WRITES NO FILE AND READS NONE: you store the block, which keeps the safety model's reliance on process death being an unambiguous end of session, and means there is no policy here about where your output lands. TWO THINGS CANNOT CROSS INTO ANOTHER JVM AND ARE DROPPED RATHER THAN CARRIED, each named in the reply against the stop point it came from: an instance filter (a JDWP object id is a WEAK reference to one object in one JVM, ADR-0022) and a thread filter (same reason, and a pool that retires idle workers invalidates it even inside one JVM). Read those lines, because a dropped filter leaves that entry BROADER than the one you exported — on a shared instance that is the difference between a diagnostic and an outage. Resolved class, method and field ids are not in the set at all; it carries the class name, line and field name you gave and re-resolves them, which is what makes a set usable against a REDEPLOYED build rather than only against the same process. trace_max_hits in the set is the budget REMAINING, not the one you originally asked for, since that is the only one a session keeps — a stop point that has spent 190 of 200 exports with 10. A disabled or spent stop point is exported with its arguments and marked, and arm_stop_points will NOT arm it.".to_string(),
            input_schema: to_val(schemars::schema_for!(ListStopPointsArgs)),
        },
        Tool {
        ...
        Tool {
            name: "debug.update_stop_point".to_string(),
            description: "Change an armed stop point's condition or hit_count IN PLACE, keeping its id and its hit tally. Works on bp_ / exc_ / watch_ / mexit_ / mon_ / bpset_ ids and on a deferred breakpoint waiting for its class. THE POINT IS THE TALLY. Changing a condition used to mean debug.clear_stop_point followed by a fresh debug.set_*, which allocated a NEW id — so a bp_ you had written down stopped meaning anything — and reset the hit count that debug.list_stop_points reports. Tightening a condition is iterative by nature: arm broad, watch what fires, narrow. The narrowing is exactly the moment you want to compare how often the broad version fired against how often the narrow one does, and that is the number the old path deleted at the moment of comparison. A bpset_ family also re-walked every loaded class and re-installed its class-load watch, which is the expensive half of arming, spent to change one string. THREE STATES PER FIELD, SPELLED OUT: condition:\"…\" sets it, clear_condition:true removes it, omitting both leaves it alone — and the same for hit_count / clear_hit_count. Passing a field and its clear_ flag together is REFUSED rather than resolved by precedence, because guessing which you meant would silently decide what the stop point fires on. A CONDITION CHANGE SENDS NO JDWP PACKET; A HIT_COUNT CHANGE SENDS SEVERAL. Conditions are evaluated on this side, so changing one is free and instant. hit_count is JDWP's Count modifier, which lives on the event request and cannot be edited — so the request is cleared and re-set carrying the new count. Your id and tally survive that; the JVM's own count restarts, and the reply says so. THE ARMING TOOLS' REFUSALS APPLY UNCHANGED, because it is their code rather than a second copy: a condition that invokes is refused under read_only exactly as at arming (ADR-0001), and an invalid one leaves the stop point exactly as it was — armed, with its previous condition. A SPENT stop point is refused (ADR-0026): it reached its hit count, the JVM has already deleted the request, and an update that reported success on one that can never fire again would read like a change and not be one. A MONITOR stop point takes no condition — debug.set_monitor_stop has no such argument — so its hit_count updates and a condition is refused with that said. A bpset_ family updates as one unit: every current member takes the new value, and so do classes that load later. This does NOT change a stop point's LOCATION — that is a different stop point, and clear + set is correct for it — nor trace, trace_expr, the trace bounds, or thread/instance filters. To silence one without changing what it decides on, use debug.toggle_stop_point.".to_string(),
            input_schema: to_val(schemars::schema_for!(UpdateStopPointArgs)),
        },
        Tool {
