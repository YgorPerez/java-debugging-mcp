...
async fn describe_method_exit_event(
    ...
    match return_value {
        Some(v) => {
            obj.insert(
                "returned".to_string(),
                json!(render_value(conn, v, None, 200, ByteRender::default()).await),
            );
        }
        None => {
    ...
    match new_value {
        Some(nv) => {
            if let Some(old) = current {
                obj.insert(
                    "old".to_string(),
                    json!(render_value(conn, &old, None, 200, ByteRender::default()).await),
                );
            }
            obj.insert(
                "new".to_string(),
                json!(render_value(conn, nv, None, 200, ByteRender::default()).await),
            );
        }
                ...
                if !slots.is_empty() {
                    if let Ok(vals) = conn.get_frame_values(thread, frame.frame_id, slots).await {
                        for ((name, _), val) in in_scope.into_iter().zip(vals.iter()) {
                            let rendered = render_value(conn, val, None, 100, ByteRender::default()).await;
                ...
                let rendered = match split_charset(e) {
                    Ok((expr_text, bytes)) => {
                        match resolve_expression(conn, Some(thread), Some(&frame), expr_text).await {
                            Ok(v) => render_value(conn, &v, Some(thread), 200, bytes).await,
