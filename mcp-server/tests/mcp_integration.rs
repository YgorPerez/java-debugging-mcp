...
fn a_per_thread_suspended_frame_reads_and_writes_but_cannot_invoke() {
    ...
    assert!(
        !sub.contains("= \"layout-adturismo\""),
        "a Map subscript INVOKES get(), which JDWP does not allow on a thread suspended this way — if \
         this ever starts working the refusal below is now a lie and both must be revisited: {sub}"
    );
    assert_contains_all(
