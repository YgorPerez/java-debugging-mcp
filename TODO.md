...
### The flake family (TEST-16/19/20/21/22) — three open

Five intermittent failures accumulated in this suite, and #38's lesson is why they were worth chasing as a
...
Two are closed by a fix, one was **rejected as a mechanism and fixed for a different reason**, and two are
open. The pattern across all five is worth more than any of them individually: **every one that was
diagnosed turned out to be the test racing for a precondition the debuggee could simply have stated.**
#54's probe now says `held=<n>`; #55's readiness now reads the agent's own banner instead of inferring
readiness from a socket. Where a cause could *not* be captured (#45), nothing was tuned — the failure was
made self-documenting instead, because a test quietly adjusted until it stops failing is a test that has
stopped looking.
