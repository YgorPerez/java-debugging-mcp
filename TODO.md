...
> **Code is cited by symbol, not by line.** `handlers.rs` is over 20,000 lines and every insertion above a
> citation silently invalidates it: an audit of the `file.rs:NNNN` references here found **13 of 16 pointing
> at unrelated code**, all of them still plausible-looking. A symbol name is greppable and does not rot, so
> the line numbers were removed rather than refreshed — refreshing only resets a clock. Same reasoning as
> `CLAUDE.md`'s rule about written-down shard numbers. The one exception is the collapsed block of original
> backlog entries at the end, which is a frozen record of what was filed and is left verbatim.
...
**38 ADRs now**, each with the rejected alternative and the evidence that settled it — why read-only is
enforced at the wire boundary, why the trace budget isn't JDWP's `Count`, why suspends have to be counted,
why an auto-disarm disables rather than deletes, why stop-point ids aren't request ids, why expansion is
opt-in, why `doctor.sh` is the lint gate, why a dump selects by name family rather than creation order, why
an object handle is weak and never pinned, why a heap query reports the pause it imposed, why a named query
suppresses its own flush, and why independent reads share one round trip.

[`docs/adr/README.md`](docs/adr/README.md) is the index and is the authority — do not re-list them here,
because a second list is a second thing to keep true. The two sections below are the operational summaries;
...
verified the same way twice: dropping its `SuspendCount` check named `(ThreadSuspendTwice, OneThread)`,
and leaving a pending step armed on the thread it releases named `(Step, OneThread)`.

FILT-7 (#91) added `ConditionEscalated` — the debugger, not the JVM, issuing the VM-wide suspend when a
condition holds. SAFE-11 (#90) added `ThreadSuspend` and `ThreadSuspendTwice`, and the fifth path. The
two arrived in parallel branches, each extending a five-state matrix, and **both states are here because
the merge kept both rather than letting one branch's count win** — a matrix that silently covers less
than it claims is the exact failure it exists to catch.

The per-thread states are run against every path including the ones that cannot fix them:
`debug.continue` is about the VM's suspend depth and deliberately does not release a held thread, so its
honest answer there is to name what it left behind.
...
`EventThread` and the **debugger** issues the VM-wide suspend when the condition holds (ADR-0020). It
...
- **Field watchpoints (WATCH-1)** — `debug.set_field_stop {class_name, field_name, modify, access}`
  ...
  **old → new** pair — the old value is read while the thread sits suspended *before* the pending
  ...
  real MCP server — 17 checks, covering static + instance modify, access-only, and the bookkeeping/error
  paths. Those ran from `examples/test_watchpoint.rs`, an ad-hoc harness **deleted by TEST-1** (`53b2632`)
  when the coverage moved into the suite proper; the successors are
  `watchpoints_report_field_writes_and_reads` and `traced_watchpoints_record_writes_without_suspending`
  in `mcp_integration.rs`, still against `examples/probes/WatchProbe.java`.
...
- **Static-method invocation + object arguments (EVAL-1, EVAL-2)** — `debug.evaluate` now calls
  ...
  **expressions** as arguments, passed by reference (`a.matches(b)`, `EvalProbe.describe(a)`,
  ...
  Validated end-to-end through the real MCP server over JSON-RPC, 19 checks. Same move as the watchpoint
  entry above: `examples/test_eval_invoke.rs` was deleted by TEST-1 (`53b2632`) and the successor is
  `evaluate_static_methods_and_object_arguments` in `mcp_integration.rs`, still against
  `examples/probes/EvalProbe.java`.
...
**Six open as of 2026-08-05, and every table in this section is history.** `gh issue list --state open` is
the authority; this count has been stale in both directions before (it once said eleven when five were open,
and said four when six were). The open set:

| issue | | |
| --- | --- | --- |
| [#45](https://github.com/YgorPerez/java-debugging-mcp/issues/45) TEST-16 · P2 · S | flake | `force_return`, ~1 run in 24, cause never captured |
...
> measurement that justified a default, the mechanism a fix turned out not to need — and several ADRs cite
> it rather than restating it. Where a "what is actually left" cell still reads as though work remains, it
> has been corrected in place and dated.

**From #17–#22's evidence — all closed.** #25, #26 and #27 shipped; #24 turned out not to need most of what
it said it needed, and #28 closed with the toolchain-pin workflow — see the shipped entries above.

| issue | why it exists | what is actually left |
| --- | --- | --- |
| [#24](https://github.com/YgorPerez/java-debugging-mcp/issues/24) TEST-8 · P1 | Successor to the closed TEST-6/#13. Every shared-instance default (`max_suspend_ms` 2000, `limit` 40, `max_frames` 8) was calibrated on loopback against probes, and the monitors-only saving was measured at 3 frames deep. | **The reading has been taken** (2026-07-27), against a real WildFly 21 running a real war, loaded to 267 threads with request workers a median 328 frames deep. `max_suspend_ms` **2000 kept**: worst dump measured 573ms, ~3.5× inside it, and the projection says it binds past ~5ms RTT — the right shape for a safety default. `max_frames` **8 kept**, with the reasoning recorded. `monitors_only`'s saving is **1.6–2.4× under load and *negative* when idle** — the predicted widening at WildFly depth was wrong in both directions, and ADR-0009 + `args.rs` now say so. It also found [#43](https://github.com/YgorPerez/java-debugging-mcp/issues/43). Left: nothing measurable. The residual "how long may we freeze a VM other people are using" is a policy call, not a reading, and the instance measured was the local isolated one — [#37](https://github.com/YgorPerez/java-debugging-mcp/issues/37) is what would make a visit to the actual 8180 permanent. |
| [#43](https://github.com/YgorPerez/java-debugging-mcp/issues/43) DUMP-3 · P1 · M | Fell out of #24's reading. A default dump against a 267-thread WildFly returned **zero application threads** — `collect_dump_rows` walks JDWP `AllThreads`, which is *creation* order, and an app server creates its request pool last, so the 40 slots went to JVM internals and Undertow selectors while 13 workers sat 328 frames deep. The header reads "40/267 thread(s)", which looks like a sample. | **Fixed** (2026-07-27, ADR-0013). A dump too big for `limit` now selects **one thread per name family** — the name with its digits collapsed, so a pool numbers into one group — round-robin across families, and prints the rows in creation order. The header states the rule and the footer names the groups it withheld, so nothing has to be inferred from the source. Cost is ~2.4× a default dump on the 267-thread shape (every thread's name and status, ~790 packets against 332), paid only when the dump truncates and bounded by `max_suspend_ms`, whose triage share is capped at half. `ChurnProbe` grew 40 immortal `io-selector-*` threads to host the test — the eight it already started last were **not** enough, because HotSpot compacts its live thread list, so short-lived predecessors stop holding a position. Left: nothing. The neighbouring defect it scoped out — `debug.list_threads` truncating in the same creation order — was filed as [#51](https://github.com/YgorPerez/java-debugging-mcp/issues/51) and **fixed** (2026-07-27) by extending this same decision to that tool rather than inventing a second rule: the listing reached 0 of `ChurnProbe`'s 8 last-started workers before and 8 of 8 after, for 103 packets against 41, on a JVM whose dump costs 381. See ADR-0013's DUMP-5 amendment. |
| [#28](https://github.com/YgorPerez/java-debugging-mcp/issues/28) LINT-2 · P2 | The #18 gate's maintenance debt: a pinned toolchain with no bump trigger, and per-crate `clippy.toml` that a third crate would not have. | **Closed.** Item 1: `.github/workflows/toolchain-pin.yml` runs the same scan against `stable` monthly **without gating** and opens an issue when the pin is behind, so the bump is scheduled work rather than a surprise (ADR-0007). Item 2: one `clippy.toml` at the workspace root now covers every crate, which works only because `doctor.sh` and the workflows set `CLIPPY_CONF_DIR` — rust-doctor drops a temporary `clippy.toml` into any member that lacks one, which would otherwise shadow it. Adding a workspace member needs nothing. Left: nothing. |
...
| issue | why it exists | what is actually left |
| --- | --- | --- |
| [#29](https://github.com/YgorPerez/java-debugging-mcp/issues/29) DISC-1 · P1 · S | No class discovery. On the shared 8180 a generated proxy or a shaded class is not findable from the source tree at all — only the debuggee knows what it loaded. | **Shipped** as `debug.list_classes`. All three named gaps were built rather than deferred: the tool, JNI-signature-to-FQN rendering, and the bound — `filter` takes `com.example.*`, `*.OrderService` or a bare substring, and the reply reports matched-against-loaded rather than dumping thousands of types. Arrays are excluded unless asked for. Left: nothing. |
| [#30](https://github.com/YgorPerez/java-debugging-mcp/issues/30) DISC-2 · P2 · S | No method listing, so a caller composing `debug.evaluate` cannot see the parameter lists they are trying to satisfy — the most intricate part of this server, driven blind. | **Shipped** as `debug.list_methods`, and the rendering went further than the ask: signatures come back as Java source types **with type parameters and arguments** where the class file carries them (`<T> List<T> firstOf(List<T>, Map<String, T>)`), which is what a caller composing `debug.evaluate` needs, since overloads resolve on the runtime types supplied. `static`/`abstract`/`native` are marked — the latter two have no body to break on. `debug.list_fields` was built alongside it for the same reason. Left: nothing. |
| [#31](https://github.com/YgorPerez/java-debugging-mcp/issues/31) DISC-3 · P2 · M | A stop point reports `class.method:412` and nothing can read line 412 — or confirm the local checkout is the deployed build, which is the assumption that wastes an hour mid-investigation. | **Shipped** as `debug.source`, built as the two halves this cell names and kept independent (ADR-0029). The JVM half needs no local files and settles whether your checkout is the code that is running; the disk half turns `class.method:line` into a bounded window. The assumption behind "confirm the local checkout is the deployed build" turned out to be **wrong and was filed separately**: `SourceFile` is a compile-time string, identical across every build of a file, so it answers *which file* and never *which build of it*. That is `debug.check_stale` (DISC-7), which compares line tables method by method. Left: nothing. |
| [#32](https://github.com/YgorPerez/java-debugging-mcp/issues/32) EVT-2 · P2 · M | Hits are discoverable only by polling, so the watchdog's 120s budget burns while nobody is looking. MCP already has the mechanism. | **Shipped.** A hit reaches the agent as an MCP `notifications/message` push (`protocol.rs`), and every constraint in the brief was honoured rather than traded away: push fires for **suspending** hits only, since a traced stop point is built to fire hundreds of times a second and would flood the transport; repeats are bounded per SAFE-8/#8; and the buffered `debug.get_last_event` stays regardless, because notifications are best-effort and a dropped one must not be the only record. Left: nothing. |
...
| [#34](https://github.com/YgorPerez/java-debugging-mcp/issues/34) REL-1 · P3 · S | Installing needs a Rust toolchain, which hands back most of the no-JVM argument for writing JDWP natively — a Java developer is the person least likely to have `cargo`. | **Shipped.** `.github/workflows/release.yml` publishes four prebuilt binaries per tag — Linux x86_64 (statically linked against musl, so glibc vintage stops mattering), macOS aarch64/x86_64, Windows x86_64 — plus a `SHA256SUMS` covering all four, which the downstream toolkit consumes as its manifest rather than assembling a URL (`docs/toolkit-contract.md`). It reuses the existing pin as this cell asked. Two things it learned since: REL-2 (#41) made the publish fail if anything in `dist/` is not a platform binary, after a SARIF file shipped as a release asset; and `scripts/release-notes.py` now writes the body, because `--generate-notes` lists merged **pull requests** and published one bare compare link for every release from v0.2.1 to v0.8.0. Left: nothing. |

**From TEST-8's method — all four shipped.** #24 was labelled `ready-for-human` because it needed the shared
8180, and most of it turned out not to. The generalisation is worth keeping and has now been paid for four
times: *"we cannot test that" is usually "we have not built the instrument"*. Four were proposed; the first
shipped immediately — `FaultRelay`, a fault-injecting JDWP proxy, which reached `resume_all_fully`'s
honest-failure tail that the coverage review had called unreachable through this tool's own API. These are
the other three, and each built the instrument its "we cannot test that" was hiding: adversarial probes
(#35), a JDK matrix (#36), and cassettes that need no JVM at all (#37) — plus #38, which fixed the
instrument that was measuring the wrong thing.

| issue | why it exists | what is actually left |
| --- | --- | --- |
| [#35](https://github.com/YgorPerez/java-debugging-mcp/issues/35) TEST-10 · P2 · S | Every probe is well-behaved, so four real debuggee states are never presented: threads dying mid-dump (the `collect_dump_rows` arm for it is unexercised), contention beyond `DeadlockProbe`'s two threads, synthetic/lambda class names, and most `ValueData` variants — which is *why* `types.rs` sits at 16.67%. | **Shipped**, and it stayed cheap as predicted — probes only. The four states this cell names are now presented by construction: threads dying mid-dump (`ChurnProbe`, whose ring of held retirements makes ZOMBIE and collected rows both exist — see #54), contention past two threads (`ContendedProbe`, `MonitorProbe`, `DeadlockProbe`, `WedgeProbe`), synthetic and lambda class names (`SyntheticProbe`), and the `ValueData` variants (`PrimitiveProbe`). The probe count is past 50. Left: nothing. |
| [#36](https://github.com/YgorPerez/java-debugging-mcp/issues/36) TEST-11 · P2 · S | The suite runs on JDK 21; **the 8180 runs JDK 11**, and nothing had ever run against it. | **Done.** The first run on 11 failed 50 of 53 — not a JDWP difference but `javac` reading source in the platform charset before JEP 400, so every probe comment with an em dash failed to compile. One `-encoding UTF-8` later (`a16bb90`), and `tests.yml` now runs the integration suite as a `fail-fast: false` matrix over **11 / 17 / 21**, each leg named, each carrying its own no-SKIP guard and a check that `$JAVA_HOME` really holds the version the leg asked for — a leg that tested the wrong JVM is as green-and-worthless as one that tested nothing. Unit tests moved to their own job so a code break does not present as three red JDK legs. **The result: 56/56 on 11, 17 and 21, locally, identical.** Capabilities, lambda and synthetic names through `decode_signature`, line and local-variable tables, the `Frames`/`INVALID_LENGTH` behaviour behind ADR-0008 — all the same on the generation the 8180 actually runs, so the substantive worry is measured and retired rather than confirmed, and **no test needs a version-gated skip**. 8 is deliberately not a leg (nothing in the estate runs it) and was never run here. A matrix still can**not** reach the `JDWP < 1.6` path — JDWP tracks the JDK, so 11 speaks 1.11; that needs `FaultRelay` or #37. **#37 took it**: a hand-edited cassette answers `JDWP 1.5` and the degraded arming now executes, with no JVM at all (ADR-0014). |
| [#37](https://github.com/YgorPerez/java-debugging-mcp/issues/37) TEST-12 · P2 · M | #24's residue is one human dump against the real instance, after which the evidence evaporates. Recording the JDWP stream once turns that visit into a permanent CI fixture — replayable with no JVM and no access, and hand-editable into shapes nothing can produce. | **Done** (ADR-0014). The two proxies were unified first, as the issue required: `Relay` is now the socket lifecycle alone and `wire_framed` the single framing, so recording is `FaultRelay`'s framing with a notebook and replay is the same `Relay` with `target_port: None` — an endpoint rather than a middleman. **The latency pump deliberately did not merge**: its chunk-level charging *is* ADR-0011's documented lower bound, and framing it would change a published measurement rather than its implementation. Cassettes are JSON, hex payloads in 32-byte lines, each exchange labelled with its JDWP command name, keyed by `(set, command, request payload)` rather than arrival order. **A miss answers nothing** — connection dropped, command named on stderr, `Drop` backstop fails the test if nobody looked — verified by deleting an exchange from the middle of a fixture (fails on the assertion) and from the shutdown traffic after every assertion had passed (fails on the backstop). Equivalence is proved rather than asserted: `disc2_method_listing` is one assertion body run by three callers — the probe test, a record-then-replay test that compares the two sets of replies with `assert_eq!`, and a JDK-free test against the checked-in file. Three tests in `mcp_integration.rs` now carry **no `#[ignore]`**; note that `scripts/integration-test.sh` passes `--ignored` and therefore does not run them. **It also closed #36's dead end**: `method_exit_on_a_jdwp_1_5_vm.json` is a three-field hand edit that makes the debuggee answer `JDWP 1.5`, and `debug.set_method_exit_stop`'s degraded arming executes for the first time — with no JVM, on a branch a JDK matrix cannot reach. **Events are not replayed**, said out loud in the module header, in every cassette and on stderr: an event answers no request, so it has no key, and a timer or a cue would be inventing when the debuggee spoke. Left: recording from the real 8180, which was always the human step this machinery exists to make worth taking. |
| [#38](https://github.com/YgorPerez/java-debugging-mcp/issues/38) TEST-13 · P2 · S | `latency_added_to_the_wire_shows_up_as_held_time_per_packet` failed 1 of 3 full runs with no code change — a wall-clock comparison whose two readings came from two separate attaches, so a scheduler hiccup on one of them read as the wire. The signature is specific and was seen again at load average 46: the **zero-latency control** inflates (5.34ms/packet observed) until it swamps the 4ms being injected. Nothing is wrong with the relay when that happens, which is what makes it dangerous — and a suite with a known-flaky member trains everyone to re-run on red. | **Done.** The round trip is now a dial on one live connection (`set_rtt`), the arms alternate, and each is scored on its fastest of three samples. **No bound was widened** — the assertion is still `0.4×rtt .. 3.0×rtt`; only the estimator changed, so the test did not buy its green by asserting less. The two failure regimes are covered by the two halves: a spike on one reading is outvoted by the fastest-of-three, and a slowdown lasting the whole test raises both arms by the same milliseconds and cancels out of their difference. Measured with both shapes run alternately in one process against the same probe, so they share their load epoch: under an injected spike, the old shape failed **8 of 8** and the new one passed **8 of 8** — with individual near samples of 9.7–20.5ms/packet, well past the 5.34 that broke the old shape. Unstressed, 44 paired rounds passed on both, the new shape's worst margin to the bound roughly double the old one's (2.68ms vs 1.53ms). Verified still load-bearing: with `set_rtt` defeated it reports a −0.03ms difference and fails. |
...
### The flake family (TEST-16/19/20/21/22/23/24/25) — three of the eight still open, plus #118

Eight intermittent failures accumulated in this suite, and #38's lesson is why they were worth chasing
as a group rather than tuning one at a time: *a suite with known-flaky members trains everyone to re-run on
red, which is how a real regression gets waved through.*

**Five of the eight are now closed** (#54, #55, #57, #65, #71); **#45, #56 and #64 remain open**, and
[#118](https://github.com/YgorPerez/java-debugging-mcp/issues/118) TEST-34 joined the family afterwards —
`launch_suspends_before_the_first_instruction` fails ~1 in 3 under CI contention and *blames `suspend=y` for
something else*, which is this family's signature defect (a message several worlds produce) rather than a
new one. It is not described in the table below, which predates it.

**#71 closed on its own third acceptance criterion, and that ending is worth copying.** It was filed from a
**two**-core arm and its own text said two cores is harsher than CI's four; the rate at four cores came in at
**0 in 44 full-suite runs**. What made the closure evidence rather than absence is the asymmetry: three
*different* flakes surfaced in those same runs, so the sampling was not too gentle — it was aimed at a fault
that contention level does not produce. The instrumentation it bought stays, and `flake-report.sh` will mark
a future sighting against it rather than `UNFILED`, so closing costs nothing. **Pre-authorising the null
result in the issue's own acceptance criteria is what made it cheap to end.**

Two were closed by a fix, one was **rejected as a mechanism and fixed for a different reason**, one was
closed as not a defect, and one closed as an arm artefact. The pattern across the diagnosed ones is worth more than any of
them individually: **every one that was explained turned out to be the test racing for a precondition the
debuggee could simply have stated.** #54's probe now says `held=<n>`; #55's readiness now reads the agent's
own banner instead of inferring readiness from a socket. Where a cause could *not* be captured (#45),
nothing was tuned — the failure was made self-documenting instead, because a test quietly adjusted until it
stops failing is a test that has stopped looking.

**A second pattern showed up once there were enough of these to compare, and it is the more actionable
one:** in four of the seven, the thing blocking diagnosis was not the rarity of the failure but a *message
several worlds produce*. #57's `the filtered stop point never recorded a throw` covered four; #45's
`caller never observed the forced value` reads as an accusation against `force_return` when an unresumed VM
is silent identically; #55 was filed on a log line **the harness itself had forged**; #65's
`Reply channel closed` was one message for four different worlds, with the real cause logged at a level
nothing enabled and then discarded. Chasing a rate is expensive and often fails — 3,573 cycles bought #45
...
| [#56](https://github.com/YgorPerez/java-debugging-mcp/issues/56) TEST-21 · P3 · S | Split out of #55, which cleared its own suspect and left this one standing: attach intermittently fails `Connection refused`, reproduced on the *unmodified* harness. | **Open.** #55 narrowed it to two mechanisms — a live handshaked session provably refuses a second connection, and `free_port`'s documented TOCTOU — and left a diagnostic: since readiness now reads *this* JVM's banner naming *this* port, a lost `free_port` race should now present as a readiness timeout quoting the JVM rather than a refusal at attach. **The next sighting distinguishes them for free**, which is why this is filed rather than patched. **15 legs since** (5 CI runs × 3 JDKs) show no refusal at any of the ~50 attach sites per run, and neither named test failed. Then the failure was *manufactured* instead of waited for, and it found two things wrong with this issue rather than with the code: the diagnostic keyed "the port is taken" on something being **listening**, which measurement shows is never true in either world — a handshaked session closes the agent's listener, so #55's own finding one step further on makes "nothing listening" the *signature* of a live session rather than of a bind failure. And the two-mechanism list omitted the likeliest third: **the JVM is simply gone**, which is not a port race at all. `diagnose_refusal` now reads `try_wait` and states one of four verdicts, each covered by a test that manufactures its world and was checked by breaking the input it depends on. A local arm added 50 clean runs across both JDKs. Left: a sighting, which will now name which world it is. |
| [#45](https://github.com/YgorPerez/java-debugging-mcp/issues/45) TEST-16 · P2 · S | `force_return` fails ~1 run in 24 and **its cause was never captured** — the reason it sat unassigned while the others were worked. | **Open — not reproducible, and deliberately not patched** (2026-07-27). 25 full-suite runs and 1,166 solo runs (**3,573 `force_return` cycles**) on JDK 11 and 25, at 32 and 64 hogs and with two suites racing for JVMs, produced no failure. Not too gentle — that arm reproduces #54 at will and broke two other tests ([#57](https://github.com/YgorPerez/java-debugging-mcp/issues/57)) — but aimed at the wrong property: every wait is 25s against a probe acting every 150ms, so starvation would need ~150×. Two hypotheses killed by reading (the deferred-arming window is closed at `handlers.rs`; the forced `String`'s id cannot be collected under `SuspendPolicy::All`). What shipped is diagnosis, not a fix: all three red paths now separate their causes — notably *"caller never observed the forced value"*, which reads as an accusation against `force_return` when **a VM that was never resumed is silent identically**. Verified by fault injection **and now by a test**: that branch lived inline in a panic message where nothing could exercise it, and is now `resume_verdict` under `a_missing_forced_value_says_whether_the_debuggee_ran_at_all`, which asserts both wordings and that they do not overlap — the same shape as the v0.5.0 bug that hid behind an assertion on an aside. **A further 24 integration legs** (8 CI runs × JDK 11/17/21) since the diagnostics landed passed it — including the two red runs, whose logs show it `... ok` — so the free sample now matches the filing rate's denominator twice over. #57's "the harness fix landed after the filing" escape **does not transfer**: the pipe deadlock needs a `tools/list` over 64 KiB, which it only exceeded when `bb6e399` added the SWAP tools, a day *after* this sighting. A lighter local arm added 30+30 clean runs on JDK 11.0.30 and 25.0.3, which is weaker than the 3,573 cycles already here and is recorded only so it is not mistaken for progress. Left: a captured failure. |
| [#57](https://github.com/YgorPerez/java-debugging-mcp/issues/57) TEST-22 · P3 · S | Fell out of #45's failed reproduction: under two concurrent suites + 32 hogs, `a_thread_filter_holds_against_a_real_pool_of_reused_threads` failed 2 of 12 and the #43/#51 dump-and-listing test 1 of 12. | **Closed as not planned** (2026-07-28) after the premise was questioned rather than the tests. **0 of 12** full-suite runs in the exact arm it was filed from and **0 of 88** individual executions reproduced it — and the first attempt to measure it was worthless for a reason worth keeping: a bare `wait` in my own harness waited on the never-exiting CPU hogs, so a completed round was reported as no rounds at all. What closed it was the timeline: `d6e1969`'s harness stdout-drain fix landed **6 hours after this was filed**, on a path both tests depend on. The instrumentation half shipped anyway; both assertions now name which world they are in. Left: nothing. |
| [#64](https://github.com/YgorPerez/java-debugging-mcp/issues/64) TEST-23 · P2 · S | `events_are_buffered_so_a_second_hit_doesnt_erase_the_first` reported `2 older event(s)` where it staged `1`, newest `seq:3`. JDK 17 leg of `a1b60c4`; 11 and 21 passed the same commit. | **Open, and the odd one out: its state was captured on the first sighting.** The assertion prints the whole reply, so there is something to reason from now rather than a soak to run — which makes it the most tractable member of this family despite being the newest. Not yet explained, and the honest reading is not settled: an extra `step` event may mean the buffer is wrong *or* that the test's expectation is, and which one decides whether anything in the product changes. **80 runs clean** (40 each on JDK 11.0.30 and 25.0.3) against a prebuilt binary, so no local reproduction — and weak evidence by construction, since the JDK it failed on is **17** and this box has neither 17 nor 21 with a `javac`. The captured message named the backlog *size* and not its contents, so the assertion now prints the whole buffer (`limit:10`, no `drain`); a healthy run is exactly `breakpoint seq:1 @44` then `step seq:2 @45`, verified by forcing the assertion red. **Three mechanisms since killed by reading**, recorded so nobody re-walks them: the deferred-arming gap cannot double-arm (`handlers.rs` clears the `CLASS_PREPARE` watch *before* arming); the class-prepare handler is idempotent (clears its watch, then `retain`s the pending list without that `bp_id`, and takes only the first `ClassPrepare` per composite); and the watchdog cannot have resumed anything, since its default is 120s (`handlers.rs`) and the whole failing suite ran in 89s. That last one is ruled out on the numbers rather than on plausibility, because the mechanism behind it is **real**: a pending single-step survives its own event and re-fires on the next resume, which `handlers.rs` states outright. It stays the best candidate if the extra event turns out to be a second *step* — which the capture could not tell from a second *breakpoint*, since a count fits `bp,bp,step` and `bp,step,step` equally. Left: a sighting that names all three events; the buffer is now printed, and `EventFault::DuplicateKind` reproduces the fingerprint on demand for comparison. |
| [#71](https://github.com/YgorPerez/java-debugging-mcp/issues/71) TEST-25 · P2 · S | `latency_added_to_the_wire_…` failed **1 of 6** full-suite runs on JDK 17.0.20 pinned to **two** cores — the first reproducible recipe in this family, and it came from the runner-emulating arm rather than a soak. | **Closed as an arm artefact** (2026-08-04), on the issue's own pre-authorized third criterion — see above. The caveat it was filed with turned out to be the answer: **two cores is harsher than CI's four**, the same arm at four cores was clean for 18 full-suite runs when filed, and that has since held for **0 in 44**. Filed because the *message* is the defect either way: `no packet cost` came from `.expect()` on a parser that returns `None` both for a dump missing its cost line and for **a reply that is not a dump at all** — verified that an error reply parses to `None` — and under contention the second is far likelier. Nothing compared a duration, so this is not the timing failure #38 fixed. The five cost/per-packet assertions now print the reply through the `head_of` helper that already existed for it. Left: nothing. The five cost/per-packet assertions still print the reply through `head_of`, so a future `no packet cost` says which of the two worlds it is. |
| [#65](https://github.com/YgorPerez/java-debugging-mcp/issues/65) TEST-24 · P3 · S | `a_probe_that_has_not_run_yet_…` failed with `Failed to list classes: Protocol error: Reply channel closed` under a second into an 8s delayed-start window — so the event loop's `read_packet` errored and dropped every pending reply. JDK 21 leg of `c168263`; 11 and 17 passed. | **Closed** (2026-07-29), shipped in v0.6.1 (`3c121a8`). The cause was known and discarded twice: `handle_incoming_packet` had the `io::Error`, logged it at `error!`, and returned a bare `bool` — and that log was off by default (`main.rs` enabled only `jdwp_mcp`) *and* discarded by the harness (`.stderr(Stdio::null())`). The cause now travels in `JdwpError::ConnectionClosed`, a lapsed reply is `ReplyTimeout` rather than a dead connection, and the harness keeps the server's stderr tail. **Adjacent to #56 and deliberately not merged**: that one is refused at connect, this one was served and then died. Left: a sighting that names the `io::Error`. Since then the whole chain is covered rather than just the event loop: `a_question_asked_after_the_debuggee_dies_names_the_lost_connection` kills a probe mid-session and asserts the cause survives the handler, the MCP serialisation and the transport — verified load-bearing by discarding the cause at source, which puts `Reply channel closed`'s successor back in the reply. Left: nothing — the v0.6.1 gate passed all three JDK legs first time, including the JDK 11 integration leg that had failed on this very flake during the v0.6.0 release. One clean run is not proof, but the fix removed the message that made it undiagnosable. |
...
moving a line is invisible. `Method.Bytecodes` (declared at `commands.rs`, still uncalled) plus
`can_get_bytecodes` (decoded at `vm.rs`, still unconsulted) would turn "probably current" into
...
18. ✅ Documentation & examples — the tool reference (now `docs/tools.md`; it was a table in `README.md` until the README was cut back to install/use/strengths), `examples/*.rs` probes, `examples/observability-debugging.md`, `docs/`. Ongoing.
...
- ~~**OBJ-1 — recursive object expansion**~~ (items 7, 9, 12, 13), with the type cache it wanted (item 8);
  ...
  and **PERF-1** (container-kind caching — measured, no gain, dropped) are closed too. *That `PERF-1` is the
  2026-07 one and is not [#100](https://github.com/YgorPerez/java-debugging-mcp/issues/100), which reuses the
  tag for round-trip batching — the id space was re-walked between eras. Where the two could be confused,
  `docs/toolkit-contract.md` and ADR-0038 mean #100.*
