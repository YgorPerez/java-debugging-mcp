...
| [0036](0036-an-invoking-trace-expr-is-refused-where-the-thread-does-not-own-the-lock.md) | A `trace_expr` that CALLS A METHOD is refused on the **opening** half of a monitor pair (`blocked`, `wait`), where the hit thread does not own the monitor in its own snapshot. Measured: the invocation outruns its budget, JDWP cannot cancel it, and the JVM **re-suspends the thread when the call finally returns** — 1.2 s after the capture path resumed it — leaving it suspended for ever with no watchdog cover. Verifying the resume was implemented first and does nothing, because the suspend arrives after there is anything left to verify. Extends ADR-0035 |
