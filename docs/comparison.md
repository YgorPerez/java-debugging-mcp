...
| A hit reaches the agent by | the buffered `debug.get_last_event`, plus a `notifications/message` **push** to a client on the `initialize` handshake — push for *suspending* hits only, since a traced stop point is built to fire hundreds of times a second (EVT-2). **A client on MCP 2026-07-28 gets no push and cannot**: that revision makes `notifications/message` request-scoped, and a hit belongs to no request, so the same text goes to stderr and the buffer is the whole answer (MCP-1, ADR-0047). Both other columns predate the stateless revision, so their push is not yet measured against it | event loop is listed as a next step | **MCP notifications**, on by default and switchable with `--notifications`, plus a buffered `debug_get_events` — the same two-route shape as this column |
