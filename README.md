...
| `debug.list_stop_points` | List active stop points (line, deferred, exception, watchpoint, method-exit) with trace budgets and thread filters — plus **`Hits: N`** on every one, including `Hits: 0`, so "armed and never fired" is a statement rather than a silence; and, for each traced one, its **measured** capture cost: mean per hit, the rate hits are arriving at, and the share of the window spent capturing |
